...
// poder hablar con todos los participantes a la vez (solucionado parcialmente -> habria que mejorarlo si queremos implementarlo)

// hacer o no hacer salas (sala general -> grandes usuarios y ejecución de la funcion que produce cambios si se logea gente)

// ERROR AL MSOTAR EL ULTIMO USUARIO SI SE APAGA EL ORDENADOR
// falla lo de avisar si esta conectado

// generar una cola de llamadas en espera al administrasdor. Indicarle al que llama cuantas personas hay delante suya que tambien necesitan ayuda. Darle la opcion de abandonar la cola (cuidado si se desconecta y estaba en la cola) 
	...
	easyrtc.setUsername("Alberto");
  	easyrtc.initMediaSource(
		function(){    // success callback
		  easyrtc.connect("nombreSala", loginExito, loginFallo);
		},
...
function performCall(otroEasyrtcId) {
  ...
  if( easyrtc.getConnectStatus(otroEasyrtcId) == easyrtc.NOT_CONNECTED ){ // FALLA
	document.getElementById("infoLlamada").style.display = "block";
  	document.getElementById("infoLlamada").textContent =easyrtc.idToName(otroEasyrtcId) + " esta ocupado en este momento.";
  }
...
easyrtc.setOnStreamClosed( function (easyrtcid) {
  ...
  document.getElementById("infoLlamada").textContent = "Llamada con " + easyrtc.idToName(easyrtcid) +" finalizada."; 
});
